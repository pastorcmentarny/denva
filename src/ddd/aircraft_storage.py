...
def save_raw_reading(reading):
    ...
    Path(f"{data_path}/{date_as_folders}").mkdir(parents=True, exist_ok=True)
    airport_raw_data = DEFAULT_PATH.format(data_path, date_as_folders,
                                           dom_utils.get_date_as_filename("aircraft", "txt", datetime.now()))
    try:
        with open(airport_raw_data, config.APPEND_WITH_READ_MODE, encoding=config.ENCODING) as aircraft_raw_file:
            json.dump(reading, aircraft_raw_file, ensure_ascii=False, indent=4)
    except Exception as exception:
        logger.error(f'Unable to save raw reading due to {exception}', exc_info=True)
...
def save_processed_data(result):
    ...
    try:
        with open(airport_processed_data, config.APPEND_WITH_READ_MODE, encoding=config.ENCODING,
                  newline=config.EMPTY) as aircraft_processed_file:
            csv_writer = csv.writer(aircraft_processed_file)

            for entry in result:
                if entry['flight'] != config.EMPTY:
        ...
        logger.error(f'Unable to save processed reading due to {exception}', exc_info=True)
