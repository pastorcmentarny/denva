...
def get_measurement() -> dict:
    try:
        ...
        print(
            f'Unable to read data from icm20948 (motion sensor) sensor due to {type(exception).__name__} throws : {exception}')
        reset()
        raise exception
...
def get_motion() -> int:
    try:
        ...
        print(f'Exception occurred while getting data {exception}', exception)
        reset()
        return 0
