...
display.text("1. Previous Day", 10, 43, scale=4)
display.text("2. Next Day", 10, 77, scale=4)
...
while True:
    ...
    if button_a == 1:  # Auto
        ...
        pressure = denva_result[config.FIELD_PRESSURE]
        ...
        display.text(f"Pico Temp: {temperature:.2f}°C,", 10, 10, scale=1)
        display.text(f"Pico Temp: {temperature:.2f}°C,", 10, 32, scale=2)
        display.text(f"Pico Temp: {temperature:.2f}°C,", 10, 64, scale=3)
        display.text(f"Pico Temp: {temperature:.2f}°C,", 10, 100, scale=4)
        display.text(f"Pico Temp: {temperature:.2f}°C,", 10, 260, scale=5)
        display.text(f"Pico Temp: {temperature:.2f}°C,", 10, 310, scale=6)
        display.update()
        gc.collect()
        led.value(OFF)
    elif button_e == 1:
